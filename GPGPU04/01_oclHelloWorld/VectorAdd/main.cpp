...
int main() {
	...
	try {
		...
		//CL_TRUE szinkronizált, amúgy aszinkron is lehetne kész event-el.
		queue.enqueueWriteBuffer(buffer1, CL_TRUE, 0, sizeof(int) * LIST_SIZE, A);
		queue.enqueueWriteBuffer(buffer1, CL_TRUE, 0, sizeof(int) * LIST_SIZE, B);
